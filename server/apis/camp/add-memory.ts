...
export default api({
  ...
  async run(ctx, input) {
    ...
    const countRows = await ctx.integrations.camp_201_db.query(
    ...
    const hasPhotos = await ctx.integrations.camp_201_db.query(
      `SELECT 1 FROM camp201_memories WHERE camper_id = $1 AND memory_type = 'photo' LIMIT 1`,
      z.object({}),
      [input.camper_id],
      { label: "Check has photos" }
    );
    const hasTexts = await ctx.integrations.camp_201_db.query(
