...
export default api({
  ...
  async run(ctx, { template_id, team_id, scored_by, scores, notes }) {
    ...
    const templateResult = await ctx.integrations.camp_201_db.query(
    ...
    await awardRubricImprovementBonus(
      ctx.integrations.camp_201_db, team_id, totalScore, template.max_total_points, cohortId
    );
    ...
    if (template_id === MINI_EBR_TEMPLATE_ID && cohortId) {
      ...
      const counselorResult = await ctx.integrations.camp_201_db.query(
      ...
      const scoreResult = await ctx.integrations.camp_201_db.query(
