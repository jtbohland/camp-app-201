...
export default function PreWorkWarningModal({
  ...
  const isProfileWarning = missingProfileFields.length > 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <Card className="max-w-md w-full mx-4 p-6 shadow-2xl">
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {isProfileWarning
                ? `You're trying to complete "${itemTitle}", but your profile isn't finished yet.`
                : `You're trying to complete "${itemTitle}", but you haven't opened all the required links yet.`
              }
            </p>
          </div>
        </div>
        ...
        {isProfileWarning && (
          <div className="bg-muted/50 rounded-lg p-4 mb-4">
            <p className="text-xs font-semibold text-foreground mb-2 flex items-center gap-1.5">
              <Icon icon="user" className="w-3.5 h-3.5 text-amber-500" />
            ...
            <div className="space-y-1.5">
              {missingProfileFields.map((field, i) => (
                <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-background border border-border">
                  <Icon icon="circle" className="w-2 h-2 text-amber-500 flex-shrink-0" />
        ...
        <div className="flex flex-col gap-2">
          {isProfileWarning ? (
            <Button onClick={() => { onClose(); navigate("/profile"); }} className="w-full bg-camp-green hover:bg-camp-green/90">
              <Icon icon="user" className="w-4 h-4 mr-1.5" />
              Go to My Profile
            </Button>
          ) : (
